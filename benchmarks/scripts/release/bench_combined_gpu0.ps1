...
$progressFile = Join-Path $searchDir "combined_gpu0_progress.txt"
$resultsFile = Join-Path $searchDir "combined_gpu0_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== Combined GPU0-only sweep (3D + tile + wave) p=4..256 (cube 49^3 FE K=2 wg=256 ticks=20 single-GPU) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(4, 8, 16, 32, 64, 128, 256)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/comb_g0_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "comb_g0_p${p}.log"
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*comb_g0_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
