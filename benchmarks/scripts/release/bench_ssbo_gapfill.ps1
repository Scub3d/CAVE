...
$progressFile = Join-Path $searchDir "ssbo_gapfill_progress.txt"
$resultsFile = Join-Path $searchDir "ssbo_gapfill_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== SSBO baseline gap-fill p=4,8,16,64 (cube 49^3 FE K=2 wg=256 ticks=20 dual-GPU, NO image mode) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(4, 8, 16, 64)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/ssbo_gap_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "ssbo_gap_p${p}.log"
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*ssbo_gap_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
