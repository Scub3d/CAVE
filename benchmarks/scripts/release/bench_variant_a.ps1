...
$progressFile = Join-Path $searchDir "variant_a_progress.txt"
$resultsFile = Join-Path $searchDir "variant_a_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== Variant A: 3D dispatch ONLY (no tile, no wave) p=4..256 (cube 49^3 FE K=2 wg=256 ticks=20 dual-GPU) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(4, 8, 16, 32, 64, 128, 256)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/varA_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "varA_p${p}.log"
    ...
    $proc = Start-Process -FilePath $exe -ArgumentList @(
        ...
        "--dual-gpu","--headless","--output",$output
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*varA_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
