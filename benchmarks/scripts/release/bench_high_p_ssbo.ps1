...
$progressFile = Join-Path $searchDir "high_p_ssbo_progress.txt"
$resultsFile = Join-Path $searchDir "high_p_ssbo_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== High-p SSBO sweep p=128..1024 (cube 49^3 FE K=2 wg=256 ticks=20 dual-GPU, NO --search-image-mode) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(128, 256, 512, 1024)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/highp_ssbo_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "highp_ssbo_p${p}.log"
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*highp_ssbo_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
