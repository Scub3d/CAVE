...
$progressFile = Join-Path $searchDir "bench_grand_progress.txt"
$resultsFile = Join-Path $searchDir "bench_grand_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== Grand cross-product sweep (RELEASE BUILD): workgroup size x chunks-per-config ===" | Out-File -FilePath $progressFile -Append -Encoding utf8
"Grid 49^3 FE K=2 cs 2-3 ticks 20 dual-GPU. Per-run timeout 600s." | Out-File -FilePath $progressFile -Append -Encoding utf8
...
foreach ($wg in $workgroupSizes) {
    foreach ($p in $partitionCounts) {
        $runIndex++
        $output = "build/Release/data/search/grand_wg${wg}_p${p}"
        $absOutput = Join-Path $repoRoot $output
        Remove-Item -Recurse -Force $absOutput -ErrorAction SilentlyContinue

        $stdoutPath = Join-Path $searchDir "grand_wg${wg}_p${p}.log"
...
# Pivot table
$pivotPath = Join-Path $searchDir "bench_grand_pivot.txt"
