...
$progressFile = Join-Path $searchDir "ssbo_optimized_progress.txt"
$resultsFile = Join-Path $searchDir "ssbo_optimized_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== SSBO optimized sweep (3D dispatch + tile loading + wave intrinsics) p=4..256 (cube 49^3 FE K=2 wg=256 ticks=20 dual-GPU) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(4, 8, 16, 32, 64, 128, 256)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/ssbo_opt_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "ssbo_opt_p${p}.log"
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*ssbo_opt_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
        "[$([DateTime]::Now.ToString('HH:mm:ss'))] FAILED p=$p (exit code $($proc.ExitCode))" | Out-File -FilePath $progressFile -Append -Encoding utf8
        continue
    }
