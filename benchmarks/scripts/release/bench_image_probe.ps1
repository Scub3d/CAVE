...
$progressFile = Join-Path $searchDir "image_probe_progress.txt"
$resultsFile = Join-Path $searchDir "image_probe_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== Image-mode probe sweep p=1..1024 (variant B) (cube 49^3 FE K=2 wg=256 ticks=20 dual-GPU --search-image-mode) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/image_p${p}"
    $absOutput = Join-Path $repoRoot $output
    Remove-Item -Recurse -Force $absOutput -ErrorAction SilentlyContinue

    $stdoutPath = Join-Path $searchDir "image_p${p}.log"
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*image_p${p}cube*" } | Select-Object -First 1
