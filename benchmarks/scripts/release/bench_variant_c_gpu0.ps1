...
$progressFile = Join-Path $searchDir "variant_c_gpu0_progress.txt"
$resultsFile = Join-Path $searchDir "variant_c_gpu0_results.csv"

"" | Out-File -FilePath $progressFile -Encoding utf8
"=== Variant C GPU0-only sweep (3D dispatch + wave intrinsics, no tile) p=4..256 (cube 49^3 FE K=2 wg=256 ticks=20 single-GPU) ===" | Out-File -FilePath $progressFile -Append -Encoding utf8

$partitionCounts = @(4, 8, 16, 32, 64, 128, 256)
$results = @()
foreach ($p in $partitionCounts) {
    $output = "build/Release/data/search/varC_g0_p${p}"
    ...
    $stdoutPath = Join-Path $searchDir "varC_g0_p${p}.log"
    "[$([DateTime]::Now.ToString('HH:mm:ss'))] starting p=$p" | Out-File -FilePath $progressFile -Append -Encoding utf8
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $proc = Start-Process -FilePath $exe -ArgumentList @(
        "--mode","search","--shape","cube","--grid","49","--neighborhood","FE",
        "--chunks-per-config","$p","--search-workgroup-size","256",
        "--max-rule-bits","2","--min-cs","2","--max-cs-range","3","--ticks","20",
        "--gpu","0","--headless","--output",$output
    ...
    $aggCandidate = Get-ChildItem -Path $searchDir -Recurse -Filter "*aggregate.jsonl" -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -like "*varC_g0_p${p}cube*" } | Select-Object -First 1
    if (-not $aggCandidate) {
    ...
    $results += [PSCustomObject]@{
        Partitions = $p; WallSeconds = $elapsed; Rows = $rowCount; TotalRunsSum = $totalRunsSum; Dups = $dups
    }
