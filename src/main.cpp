...
int main(int argc, char **argv)
{
    ...
    options.add_options()
        ...
        ("chunks-per-config", "Number of partition copies per (grid × density × seed) chunk; partitions split the survival rule range upfront for parallelism (search mode). Default 64 — empirical sweet spot at 49^3 FE K=2", cxxopts::value<int>()->default_value("64"))
        ("search-workgroup-size", "Threads per workgroup for the simulation compute kernel (search mode). Valid: 32, 64, 128, 256, 512", cxxopts::value<int>()->default_value("256"))
